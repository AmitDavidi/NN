...
##HOW TO DOWNLOAD AND UNZIP - note
...
def load_dataset(dataset, path):
    ...
    # dataset is either train or test - the folder should have 2 children:
    # folder/train
    # folder/test
    cur_path = os.path.join(path, dataset)

    labels = os.listdir(cur_path)  # list of labels - names of folders

    for label in labels:  # for every label in the folder
        for file in os.listdir(os.path.join(cur_path, label)):  # for every filer inside train/label folder
            # read every image(file) in this folder
            path_to_file = os.path.join(cur_path, label, file)
            # print(path_to_file)
...
def main():
    ...
    model.set(optimizer=Optimizer_Adam(decay=1e-3),
              loss=Loss_CategoricalCrossentropy(),
              accuracy=Accuracy_Categorical())
    model.finalize()
    model.train(X, y, epochs=10, batch_size=128, print_every=100, validation_data=(X_test, y_test))
    ...
    confidences = model.predict(X_test[0:15])
    pred = model.output_layer_activation.predictions(confidences)
    
    
    for prediction, true in zip(pred, y_test[0:15]):
        print("prediction: " + fashion_mnist_labels[prediction])
        print("correct: " + fashion_mnist_labels[true])
        print()
    
    
    # make it fit to our model - gray - 28X28
